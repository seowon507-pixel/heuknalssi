const ADMINISTRATIVE_AREAS = Object.freeze([
  {
    province: "서울특별시",
    aliases: ["서울"],
    districts: [
...
function provinceMatches(area, query) {
  return [area.province, ...area.aliases].some(
    (name) => name === query || name.startsWith(query),
  );
}

export function suggestAdministrativeAddresses(value, { limit = 12 } = {}) {
  const query = normalize(value);
  if (query.length < 2) return [];

  const matchingProvince = ADMINISTRATIVE_AREAS.find((area) =>
    provinceMatches(area, query),
  );
  if (matchingProvince) {
    const suggestions =
      matchingProvince.districts.length > 0
        ? matchingProvince.districts.map(
            (district) => `${matchingProvince.province} ${district}`,
          )
        : [matchingProvince.province];
    const exactProvinceName = [
      matchingProvince.province,
      ...matchingProvince.aliases,
    ].includes(query);
    return exactProvinceName ? suggestions : suggestions.slice(0, limit);
  }

  const allAddresses = ADMINISTRATIVE_AREAS.flatMap((area) => {
    const districts = area.districts.length > 0 ? area.districts : [""];
    return districts.map((district) => ({
      displayName: [area.province, district].filter(Boolean).join(" "),
      searchNames: [area.province, ...area.aliases].map((province) =>
  ...
  return allAddresses
    ...
    .sort((left, right) => {
      const leftStarts = left.searchNames.some((name) => name.startsWith(query))
        ? 0
        : 1;
      const rightStarts = right.searchNames.some((name) => name.startsWith(query))
        ? 0
        : 1;
      return (
