...
### R10 현재 제공 범위 — 재배 중 생육 점검

현재 릴리스는 `재배 중 생육 점검`만 사용자 기능으로 제공한다. 새 농장
설정은 이 경로를 기본 선택하고, 농장 위치·작물·필요한 작물별 재배
환경을 확인한 다음 선택한 모든 작물의 현재 생육 상태를 확인한다.
생육 상태는 현재 날짜 기준 예상값을 기본 선택하되 실제 상태가 다르면
사용자가 변경할 수 있다.

`재배 전 환경 분석(LAND_SEARCH)`은 추후 업데이트 예정이다. 백엔드
계약이나 과거 시안이 존재하더라도 현재 완성 기능, 지원 기능 또는 시연
가능 기능으로 표현하지 않는다. 시작 화면에서는 선택할 수 없는 예정
항목으로 구분한다.

#### R10 측정 가능한 수용 조건

- 새 농장 설정을 열면 `재배 중 생육 점검`이 기본 선택되어 있다.
- 작물을 고른 뒤 작물별 재배 환경을 확인하고, 다음 단계에서 선택한 모든
  작물의 생육 상태를 확인한다.
- `재배 전 환경 분석`은 `추후 업데이트 예정`으로 표시되며 선택하거나
  분석을 실행할 수 없다.
- 저장된 재배 중 농장 설정을 다시 불러와도 생육 상태가 분석 요청에서
  누락되지 않는다.
