...
서로 의미가 다른 네 축을 하나의 종합점수로 합치지 않는다. 사용자에게 보여줄 결론과 다음 행동은 이 결과축을 입력으로 받는 **결정론적 규칙 엔진**이 만든다. LLM은 계산이나 판정을 바꾸지 않고, 이미 확정된 사실을 쉬운 문장으로 바꾸는 역할만 맡는다.
...
이 값은 개별 변수 보조표시이며 5종 공통 대표점수나 기후·토양 종합점수가 아니다.
...
- 표본수
- 자료기간
- 온도·습도·CO₂ 등 확인된 지표의 중앙값과 25·75백분위
- 현재 사용자 시설 측정값이 아니라는 고지
...
```ts
interface SmartfarmBenchmarkMetric {
  metric: string;
  unit: string;
  median: number;
  p25: number;
...
interface SmartfarmBenchmarkResult {
  crop: Crop;
  cultivationMode: "FACILITY_SOIL" | "FACILITY_HYDRO";
  sampleFarmSeasonCount: number;
  period: { from: string; to: string };
  metrics: SmartfarmBenchmarkMetric[];
  limitationTemplateId: string;
}
```
...
interface AnalysisResult {
  ...
  smartfarm: ModuleSummary<SmartfarmBenchmarkResult> | null;
  satellite: ModuleSummary<SatelliteResult> | null;
  dataSources: SourceDisclosure[];
  capabilities: {
    smartfarm: "ENABLED" | "DISABLED" | "UNSUPPORTED";
...
- 기후·토양·예보·스마트팜·위성이 잘못 합산되는 경로가 없다.
...
> 흙날씨.진단의 로직은 “정확해 보이는 한 점수”를 만드는 것이 아니라, 서로 다른 데이터의 의미와 한계를 보존한 채 사용자가 다음에 확인할 행동을 결정적으로 제시해야 한다.
